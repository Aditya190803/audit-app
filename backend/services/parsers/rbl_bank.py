...
class RBLBankParser(BaseParser):
    ...
    HEADER_PATTERNS = [r"date", r"narration", r"withdrawals?", r"deposits?", r"balance"]

    COLUMN_KEYWORDS = {
        "date": [r"date"],
        "description": [r"narration", r"description", r"particulars"],
        "debit": [r"withdrawals?", r"debit"],
        "credit": [r"deposits?", r"credit"],
        "balance": [r"balance"],
    }

    BANK_ID_PATTERN = re.compile(r"\brbl\s+bank\b|\bratn0", re.IGNORECASE)
    ...
    def _extract_row(self, row: List[str], col_indices: Dict[str, int]) -> Optional[Dict[str, Any]]:
        ...
        if amount is None:
            return None
        party = self._extract_party_rbl(description) or self._extract_party_from_description(description)
        return {
            "date": date,
            "amount": amount,
    ...
    @classmethod
    def _extract_party_rbl(cls, description: str) -> str:
